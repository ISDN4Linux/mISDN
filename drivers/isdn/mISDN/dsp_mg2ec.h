...
struct echo_can_state {
	...
	/* absolute time - aka. sample number index - essentially the number
	 * of samples since this can was init'ed */
	int i_d;
	...
	/* ----------------------------------- */
	/* reference signal power estimate - aka. Average absolute value of
	 * y(k) */
	int Ly_i;
	/* ... */
	int Lu_i;
...
echo_can_update(struct echo_can_state *ec, short iref, short isig)
{
	...
	/* Update the Far-end receive signal circular buffers and
	 * accumulators */
	/* ------------------------------------------------------ */
	...
	if (isig == 0) {
		 ...
		 * we should pass it through uncancelled, as we are likely
		 * on hold */
		u = isig;
	} else {
		if (rs < -32768) {
		...
		/* eq. (3): compute the output value (see figure 3) and the
		 * error
		 * note: the error is the same as the output signal when
	...
	if (ec->start_speech_d != 0) {
		if (ec->i_d > (DEFAULT_T0 + ec->start_speech_d)*(SAMPLE_FREQ)) {
			ec->beta2_d = max_cc_float(MIN_BETA, DEFAULT_BETA1
				* exp((-1/DEFAULT_TAU)*((ec->i_d/
				(float)SAMPLE_FREQ) - DEFAULT_T0
				- ec->start_speech_d)));
		}
	} else {
	...
	*/
	...
	if (!ec->HCNTR_d &&	/* no near-end speech present */
		...
		if (ec->Lu_i > MIN_UPDATE_THRESH_I) {
			/* there is sufficient energy above the noise floor
...
#endif
			for (k = 0; k < ec->N_d; k++) {
				/* eq. (7): compute an expectation over M_d
				 * samples */
				int grad2;
