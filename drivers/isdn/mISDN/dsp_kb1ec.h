...
#define ZMALLOC(a)	zmalloc(a)
#define FREE(a) free(a)
#endif

/* Uncomment to provide summary statistics for overall echo can performance
   every 4000 samples */
/* #define MEC2_STATS 4000 */

/* Uncomment to generate per-sample statistics - this will severely degrade
...
struct echo_can_cb_s {
	/* Pointer to the relative 'start' of the buffer */
	int idx_d;
	/* The absolute size of the buffer */
	int size_d;
	/* The actual sample -  twice as large as we need, however we do store
	   values at idx_d and idx_d+size_d */
	short *buf_d;
};

/* Echo canceller definition */
struct echo_can_state {
	/* an arbitrary ID for this echo can - this really should be settable
	   from the calling channel... */
	int id;

	/* absolute time - aka. sample number index - essentially the number of
	   samples since this can was init'ed */
	int i_d;
	...
	/* ----------------------------------- */
	/* reference signal power estimate - aka.
...
static inline void init_cb_s(struct echo_can_cb_s *cb, int len, void *where)
{
...
static inline void add_cc_s(struct echo_can_cb_s *cb, short newval)
{
	/* Can't use modulus because N+M isn't a power of two (generally) */
	cb->idx_d--;
	if (cb->idx_d < (int)0)
		/* Whoops - the pointer to the 'start' wrapped around so reset
		   it to the top of the buffer */
		cb->idx_d += cb->size_d;
...
static inline short get_cc_s(struct echo_can_cb_s *cb, int pos)
{
	/* Load two copies into memory */
...
echo_can_update(struct echo_can_state *ec, short iref, short isig)
{
	...
	ec->y_tilde_i -= abs(get_cc_s(&ec->y_s,
		(1 << DEFAULT_ALPHA_YT_I) - 1)) >> DEFAULT_ALPHA_YT_I;
	...
	/* Update the Near-end hybrid signal circular buffers and
	   accumulators
	   ------------------------------------------------------ */
	/* Delete the oldest sample from the power estimate accumulator */
	...
	/* Push a copy of the current short-time average of the far-end receive
	   signal into it's circular buffer */
	...
	if (ec->start_speech_d != 0) {
		if (ec->i_d > (DEFAULT_T0 + ec->start_speech_d)*(SAMPLE_FREQ)) {
			ec->beta2_d = max_cc_float(MIN_BETA, DEFAULT_BETA1
				* exp((-1/DEFAULT_TAU)*((ec->i_d/(float)
					SAMPLE_FREQ) - DEFAULT_T0
					- ec->start_speech_d)));
		}
	} else {
	...
	*/
	...
	if (ec->y_tilde_i > ec->max_y_tilde) {
		...
		ec->max_y_tilde = MAX16(ec->y_tilde_s.buf_d
			+ ec->y_tilde_s.idx_d, ec->N_d, &ec->max_y_tilde_pos);
	}
	...
	if (((ec->s_tilde_i >> (DEFAULT_ALPHA_ST_I - 1)) > ec->max_y_tilde)
		...
		printk(KERN_INFO "Reset near end speech timer with: s_tilde_i "
			"%d, stmnt %d, max_y_tilde %d\n", ec->s_tilde_i,
		...
		/* otherwise, if it's still non-zero, decrement the Hangover
		   counter by one sample */
#ifdef MEC2_STATS
		++ec->cntr_nearend_speech_frames;
#endif
		ec->HCNTR_d--;
	...
	if (!ec->HCNTR_d && 		/* no near-end speech present */
		!(ec->i_d % DEFAULT_M)) {	/* we only update on every
						   DEFAULM_M'th sample from
						   the stream */
			if (ec->Lu_i > MIN_UPDATE_THRESH_I) {
				/* there is sufficient energy above the noise
				   floor to contain meaningful data */
...
#endif
				for (k = 0; k < ec->N_d; k++) {
					/* eq. (7): compute an expectation
					 over M_d samples */
					int grad2;
					grad2 = CONVOLVE2(ec->u_s.buf_d
						+ ec->u_s.idx_d,
						ec->y_s.buf_d
						+ ec->y_s.idx_d + k,
						DEFAULT_M);
...
#ifdef MEC2_STATS_DETAILED
				printk(KERN_INFO "insufficient signal to "
					"update coefficients ec->Lu_i %5d < "
...
#ifdef MEC2_STATS_DETAILED
	if (ec->HCNTR_d == 0)
		printk(KERN_INFO "possibily correcting frame with ec->Ly_i "
...
#ifdef MEC2_STATS_DETAILED
		printk(KERN_INFO "aggresively correcting frame with ec->Ly_i "
...
#ifdef MEC2_STATS_DETAILED
			printk(KERN_INFO "correcting frame with ec->Ly_i %9d "
	...
	*/
	...
	if ((ec->i_d % MEC2_STATS) == 0) {
		...
		if (ec->cntr_coeff_missedupdates > 0)
			ec->avg_Lu_i_toolow = (int)(ec->avg_Lu_i_toolow
				/ ec->cntr_coeff_missedupdates);
		else
		...
		if (ec->cntr_coeff_updates > 0)
			ec->avg_Lu_i_ok =
				(ec->avg_Lu_i_ok / ec->cntr_coeff_updates);
		else
			ec->avg_Lu_i_ok = -1;

		printk(KERN_INFO "%d: Near end speech: %5d Residuals "
			"corrected/skipped: %5d/%5d Coefficients updated "
			"ok/low sig: %3d/%3d Lu_i avg ok/low sig %6d/%5d\n",
			ec->id,
			ec->cntr_nearend_speech_frames,
			ec->cntr_residualcorrected_frames,
			ec->cntr_residualcorrected_framesskipped,
