...
l1oip_socket_open(l1oip_t *hc)
{
	...
	if ((hc->socket_pid =
	    kernel_thread(l1oip_socket_thread, hc, CLONE_KERNEL)) < 0) {
...
l1oip_keepalive_bh(struct work_struct *work)
{
	...
	if (debug & (DEBUG_L1OIP_MSG|DEBUG_L1OIP_SOCKET))
		printk(KERN_DEBUG "%s: keepalive bh called, sending empty "
			"frame on dchannel\n", __FUNCTION__);
...
l1oip_keepalive(void *data)
{
	...
	if (debug & (DEBUG_L1OIP_MSG|DEBUG_L1OIP_SOCKET))
		printk(KERN_DEBUG "%s: keepalive timer expired, scheduling "
	...
	case PH_ACTIVATE_REQ:
		if (debug & (DEBUG_L1OIP_MSG|DEBUG_L1OIP_SOCKET))
			printk(KERN_DEBUG "%s: PH_ACTIVATE channel %d (1..%d)\n"
				, __FUNCTION__, dch->slot, hc->b_num+1);
		skb_trim(skb, 0);
		if (test_bit(FLG_ACTIVE, &dch->Flags))
...
handle_bmsg(struct mISDNchannel *ch, struct sk_buff *skb)
{
	...
	case PH_ACTIVATE_REQ:
		if (debug & (DEBUG_L1OIP_MSG|DEBUG_L1OIP_SOCKET))
			printk(KERN_DEBUG "%s: PH_ACTIVATE channel %d (1..%d)\n"
				, __FUNCTION__, bch->slot, hc->b_num+1);
 ...
 */
static int
init_card(l1oip_t *hc, int pri, int bundle)
{
	struct dchannel	*dch;
	struct bchannel	*bch;
	int		ret;
	...
	add_timer(&hc->keep_tl);
	INIT_WORK(&hc->tqueue, (void *)l1oip_keepalive_bh);
	...
	while(type[l1oip_cnt] && l1oip_cnt<MAX_CARDS) {
		...
		if (debug & DEBUG_L1OIP_INIT)
			printk(KERN_DEBUG "%s: interface %d is %s with %s.\n",
				__FUNCTION__, l1oip_cnt, pri?"PRI":"BRI",
