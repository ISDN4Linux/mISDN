...
static inline void
init_cc(struct echo_can_state *ec, int N, int maxy, int maxu)
{
	void *ptr = ec;
	unsigned long tmp;
...
echo_can_update(struct echo_can_state *ec, short iref, short isig)
{
	...
	ec->y_tilde_i -= abs(get_cc_s(&ec->y_s, (1 << DEFAULT_ALPHA_YT_I) - 1))
		>> DEFAULT_ALPHA_YT_I;
	...
	if (ec->start_speech_d != 0) {
		if (ec->i_d > (DEFAULT_T0 + ec->start_speech_d)*(SAMPLE_FREQ)) {
			ec->beta2_d = max_cc_float(MIN_BETA,
				DEFAULT_BETA1 * exp((-1/DEFAULT_TAU)*
	...
	*/
	...
	two_beta_i = (ec->beta2_i * Py_i) >> 15; /* Fixed point version,
						    inverted */
	if (!two_beta_i)
		two_beta_i++;
...
#ifdef AGGRESSIVE_TIMELIMIT /* This allows the aggressive suppressor to turn
			       off after set amount of time */
	if (ec->i_d > AGGRESSIVE_TIMELIMIT) {
		if ((ec->HCNTR_d == 0)
			&& ((ec->Ly_i/(ec->Lu_i + 1)) > DEFAULT_SUPPR_I)) {
			u = u * (ec->Lu_i >> DEFAULT_SIGMA_LU_I)
				/ ((ec->Ly_i >> (DEFAULT_SIGMA_LY_I + 2)) + 1);
		}
	} else {
#endif
		if ((ec->HCNTR_d < AGGRESSIVE_HCNTR)
			&& (ec->Ly_i > (ec->Lu_i << 1))) {
			u = u * (ec->Lu_i >> DEFAULT_SIGMA_LU_I)
