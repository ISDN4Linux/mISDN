...
static u8 *table_com = NULL;
static u16 *table_dec = NULL;
...
l1oip_4bit_alloc(int ulaw)
{
	...
	table_com = (u8 *)vmalloc(65536);
	table_dec = (u16 *)vmalloc(512);
	...
	memset(table_com, 0x00, 65536); // clear bits
	i1 = 0;
	while(i1 < 256) {
		if (ulaw)
			c = ulaw_to_4bit[i1];
	...
	memset(table_dec, 0x00, 512); // clear bits
	i1 = 0;
	while(i1 < 16) {
		if (ulaw)
