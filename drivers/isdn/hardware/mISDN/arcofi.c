...
arcofi_fsm(channel_t *dch, int event, void *data) {
	...
	if (dch->debug & L1_DEB_MONITOR) {
		mISDN_debugprint(&dch->inst,
		    "arcofi state %d event %d", isac->arcofi_state, event);
	}
	...
	case ARCOFI_RECEIVE:
		if (event == ARCOFI_RX_END) {
			...
			if (isac->arcofi_list->next) {
				...
				if (test_and_clear_bit(FLG_ARCOFI_TIMER,
				    &dch->Flags)) {
					del_timer(&isac->arcofitimer);
				}
				isac->arcofi_state = ARCOFI_NOP;
	...
	default:
		mISDN_debugprint(&dch->inst,
		    "Arcofi unknown state %x", isac->arcofi_state);
		return (2);
	}
	return (0);
}
