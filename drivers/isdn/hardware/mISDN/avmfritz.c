...
static u8
ReadISAC(struct fritzcard *fc, u8 offset)
{
...
static void
WriteISAC(struct fritzcard *fc, u8 offset, u8 value)
{
	u8 idx = (offset > 0x2f) ? AVM_ISAC_REG_HIGH : AVM_ISAC_REG_LOW;
...
static void
ReadISACfifo(struct fritzcard *fc, u8 *data, int size)
{
...
static void
WriteISACfifo(struct fritzcard *fc, u8 *data, int size)
{
	outb(AVM_ISAC_FIFO, fc->addr + CHIP_INDEX);
...
static u8
fcpci2_read_isac(struct fritzcard *fc, u8 offset)
{
	outl(offset, fc->addr + AVM_ISACSX_INDEX);
	return 0xff & inl(fc->addr + AVM_ISACSX_DATA);
}

static void
fcpci2_write_isac(struct fritzcard *fc, u8 offset, u8 value)
{
	outl(offset, fc->addr + AVM_ISACSX_INDEX);
	outl(value, fc->addr + AVM_ISACSX_DATA);
}

static void
fcpci2_read_isac_fifo(struct fritzcard *fc, u8 *data, int size)
{
	int i;

	outl(0, fc->addr + AVM_ISACSX_INDEX);
	for (i = 0; i < size; i++)
		data[i] = 0xff & inl(fc->addr + AVM_ISACSX_DATA);
}

static void
fcpci2_write_isac_fifo(struct fritzcard *fc, u8 *data, int size)
{
	int i;

	outl(0, fc->addr + AVM_ISACSX_INDEX);
	for (i = 0; i < size; i++)
		outl(data[i], fc->addr + AVM_ISACSX_DATA);
}
...
setup_fritz(struct fritzcard *fc)
{
	...
	case AVM_FRITZ_PCI:
		...
		fc->isac.read_reg = (read_reg_t *)&ReadISAC;
		fc->isac.write_reg = (write_reg_t *)&WriteISAC;
		fc->isac.read_fifo = (fifo_func_t *)&ReadISACfifo;
		fc->isac.write_fifo = (fifo_func_t *)&WriteISACfifo;
		fc->isac.type = ISAC_TYPE_ISAC;
		break;
	case AVM_FRITZ_PCIV2:
		val = inl(fc->addr);
		...
		fc->isac.read_reg = (read_reg_t *)&fcpci2_read_isac;
		fc->isac.write_reg = (write_reg_t *)&fcpci2_write_isac;
		fc->isac.read_fifo = (fifo_func_t *)&fcpci2_read_isac_fifo;
		fc->isac.write_fifo = (fifo_func_t *)&fcpci2_write_isac_fifo;
		fc->isac.type = ISAC_TYPE_ISACSX;
		break;
	default:
