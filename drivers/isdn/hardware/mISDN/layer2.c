...
ph_data_indication(layer2_t *l2, mISDN_head_t *hh, struct sk_buff *skb) {
	...
	if (test_bit(FLG_LAPD, &l2->flag)) {
		...
		if ((psapi & 1) || !(ptei & 1)) {
			printk(KERN_WARNING
			    "l2 D-channel addr:%x frame wrong EA0/EA1\n",
			    l2->inst.id);
			return ret;
		}
		psapi >>= 2;
...
l2_manager(void *data, u_int prim, void *arg) {
	...
	if (err) {
		if (debug & 0x1)
			printk(KERN_WARNING
			    "l2_manager prim(%x) l2 no instance\n", prim);
		return err;
	}
	switch (prim) {
...
Isdnl2_cleanup(void)
{
	...
	if ((err = mISDN_unregister(&isdnl2))) {
		printk(KERN_ERR
		    "Can't unregister ISDN layer 2 error(%d)\n", err);
	}
