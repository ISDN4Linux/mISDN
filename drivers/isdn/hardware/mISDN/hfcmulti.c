...
init_e1_port(struct hfc_multi *hc, struct hm_map *m, int pt)
{
	...
	if (port[Port_cnt] & 0x0002) {
		if (debug & DEBUG_HFCMULTI_INIT)
			printk(KERN_DEBUG "%s: PORT 2MBit raw mode "
			     "E1: card(%d) port(%d)\n", __func__,
			     HFC_cnt + 1, 1);
		...
		for (ch = 0; ch <= 31; ch++) {
			...
			if ((maskcheck & hc->bmask[pt])
			 || (dmask[E1_cnt] & hc->bmask[pt])) {
				printk(KERN_INFO "HFC-E1 #%d has overlapping "
					"B-channels on fragment #%d\n",
					E1_cnt + 1, pt);
				return -EINVAL;
			}
			maskcheck |= hc->bmask[pt];
			printk(KERN_INFO "HFC-E1 #%d uses D-channel on slot "
				"%d and a B-channel map of 0x%08x\n",
