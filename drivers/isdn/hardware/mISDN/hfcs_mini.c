...
hfcsmini_ph_command(channel_t * dch, u_char command)
{
	...
	case HFC_L1_ACTIVATE_TE:
		...
		write_hfcsmini(hw, R_ST_WR_STA,
		    (M_ST_LD_STA | (M1_ST_SET_STA*4)));
...
hfcsmini_bh_handler(unsigned long ul_hw)
{
	...
	if (hw->misc_irq.bit.v_ti_irq) {
		...
		hw->fifo_irq.reg |= ((read_hfcsmini(hw, R_FILL) ^
...
init_hfcsmini(hfcsmini_hw * hw)
{
	...
	if (hw->portmode & PORT_MODE_TE) {
		...
		/* NT mode */
		hw->st_ctrl0.reg = 4;
		write_hfcsmini(hw, R_ST_CLK_DLY,
		    ((M1_ST_SMPL * 0x6) | (M1_ST_CLK_DLY*0xC)));
...
setup_instance(hfcsmini_hw * hw)
{
	...
	if (request_irq(hw->irq, hfcsmini_interrupt, SA_SHIRQ,
	    "HFC-S mini", hw)) {
