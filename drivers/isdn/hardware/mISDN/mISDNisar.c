...
send_mbox(struct isar_hw *isar, u8 his, u8 creg, u8 len, u8 *msg)
{
	...
	if (!msg)
		msg = isar->buf;
	if (msg && len) {
		isar->write_fifo(isar->hw, msg, len);
...
rcv_mbox(struct isar_hw *isar, u8 *msg)
{
	...
	if (msg && isar->clsb) {
		isar->read_fifo(isar->hw, msg, isar->clsb);
		if (*debug & DEBUG_HW_BFIFO) {
			int l = 0;
...
load_firmware(struct isar_hw *isar, const u8 *buf, int size)
{
	...
	if (!(*debug & DEBUG_HW_FIRMWARE_FIFO)) {
		fdebug = *debug & ~DEBUG_HW_BFIFO;
		debug = &fdebug;
	}
...
u32
mISDN_isar_init(struct isar_hw *isar, void *hw, u32 *deb)
{
	u32 ret, i;

	if (!debug) /* only first HW module set debug */
		debug = deb;
	isar->hw = hw;
	for (i = 0; i < 2; i++) {
		isar->ch[i].bch.nr = i + 1;
		isar->ch[i].bch.debug = *deb;
