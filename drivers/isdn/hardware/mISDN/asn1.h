	...
	if (p < end) { \
		if (((the_tag) &~ ASN1_TAG_OPT) == ASN1_NOT_TAGGED) { \
			...
			if ((the_tag) & ASN1_TAG_EXPLICIT) { \
				if ((u_char)(((the_tag) & 0xff) | \
				    (ASN1_TAG_CONTEXT_SPECIFIC | \
				    ASN1_TAG_CONSTRUCTED)) == *p) { \
					int xtag, xlen; \
					CallASN1(ret, p, end, \
					    ParseTag(p, end, &xtag)); \
					CallASN1(ret, p, end, \
...
#define XSEQUENCE(todo, act_tag, the_tag) \
    XSEQUENCE_1(todo, act_tag, the_tag, -1)
#define XSEQUENCE_OPT(todo, act_tag, the_tag) \
    XSEQUENCE_OPT_1(todo, act_tag, the_tag, -1)
...
#define XCHOICE(todo, act_tag, the_tag) \
    XCHOICE_1(todo, act_tag, the_tag, -1)
