...
static inline int CONVOLVE(const int *coeffs, const short *hist, int len)
{
	 ...
	 *  Add mm2 into mm0 (without saturation, alas).
	 *  Now we have two half-results.
...
static inline void UPDATE(volatile int *taps, const short *history,
    const int nsuppr, const int ntaps)
{
	int i;
	int correction;
	for (i = 0; i < ntaps; i++) {
...
static inline void UPDATE2(volatile int *taps, volatile short *taps_short,
    const short *history, const int nsuppr, const int ntaps)
{
	int i;
	int correction;
#if 0
...
static inline int CONVOLVE2(const short *coeffs, const short *hist, int len)
{
	...
	/*
	 *  For every set of eight...
	 *  Load in eight coefficients and eight historic samples,
	 *  multliply add and accumulate the result
	 */
	__asm__ (
		"pxor %%mm4, %%mm4;\n"
		"mov %1, %%edi;\n"
...
static inline void UPDATE(int *taps, const short *history,
    const int nsuppr, const int ntaps)
{
	int i;
	int correction;
...
static inline void UPDATE2(int *taps, short *taps_short,
    const short *history, const int nsuppr, const int ntaps)
{
	int i;
	int correction;
